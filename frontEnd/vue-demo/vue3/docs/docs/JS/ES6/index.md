...
支持更多语法，使用更方便，增强工程性。
:::tip
`Let，const，for...in，for...of，(...)展开运算符，()=>{}箭头函数，map()，forEach()，filter()，reduce()，push()，pop()，shift()，unshift()，splice(i,n)，concat()，split(),indexOf()，find()`<br>
1.相同点：<br>
都会循环遍历数组中的每一项；
...
var重复声明 ，let防止重复声明，变量。const 防止重复声明，常量。
...
ES6 新增了let命令，用来声明变量。它的用法类似于var，但是所声明的变量，只在let命令所在的代码块内有效。ES6的let的作用域——块级 {}
...
以for为例，var 重复声明，不能限制修改，为了这个问题需要包一层（早期的闭包原理），函数级。	let 变量，会有作用域

```js
let aBtn=document.getElementsByTagName('input'); //X3
// # 函数级作用域
...
修正this： 箭头函数没有this，所以需要通过查找作用域链来确定this的值。这就意味着如果箭头函数被非箭头函数包含，this绑定的就是最近一层非箭头函数的this。
1.如果有且仅有一个参数，()也可以不写  
...
```js
// 参数展开  数组展开  json展开 展开
// ...a会把第三位以后的收集起来。注意（...a只能在最后）
Show Function(x,y,z,...a){}  show(12,2,8,8,2,41,6) 
// let arr2=[...arr,...arr1]  (默认是字符串连接)
Let arr=[12,5,9]    console ...arr ； let arr1=[52,8,6]  
//（有多少都会合并）
Let json{a:1,b:13,c:14}    let json2={d:999,...json}   
```
## map

`map()`速度比forEach()快,不会对空数组进行检测。会返回一个新数组，不对原数组产生影响。
...
`forEach()`用于遍历数组的每个元素，将元素传给回调函数。 不会产生新数组，返回undefined，空数组是不会调用回调函数的。
...
`filter()`会返回一个新数组，不对原数组产生影响。筛选出数组中符合条件的项，组成一个新数组。
...
`reduce()`有4个参数（初始值(计算结束的返回值)、当前元素、当前元素的索引、当前元素所属的数组）。
第一个参数为初始值，方法是让数组中的前项和后项做某种计算，并累计最终值。
```js
...
`export`导出，与`import` 结合使用。
`export`加上`default`意指默认接口的意思，在一个文件里面默认的只能有一个 其区别就是{} 在export中 引入需要用{}来盛放。
```js
//变量
export let a=12;
export const a=12;
...
`import`导入，与`export` 结合使用。
`import`加上`default`意指默认接口的意思，在一个文件里面默认的只能有一个 其区别就是{} 在export中 引入需要用{}来盛放。
```js
// 引入所有成员 到 mod1.a mod1.b
...
`class`是一个语法糖，其底层还是通过 构造函数 去创建的。所以它的绝大部分功能，ES5 都可以做到。新的class写法只是让对象原型的写法更加清晰、更像面向对象编程的语法而已。
```js
 //常规写法
